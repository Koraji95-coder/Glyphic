...
export function CaptureToolbar({ onFullscreen, captureCount = 0 }: CaptureToolbarProps) {
  ...
  return (
    <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[10000] flex items-center gap-1 bg-zinc-900/95 backdrop-blur-2xl border border-zinc-700 shadow-2xl rounded-3xl px-3 py-2">
      {/* Mode buttons */}
      {modes.map(({ mode, icon: Icon, label, shortcut }) => (
        <button
          ...
          className={`flex items-center gap-2 px-5 py-2 rounded-3xl text-sm font-medium transition-all ${
            captureMode === mode
      ...
      {captureCount > 0 && (
        <>
          <div className="w-px h-8 mx-2 bg-zinc-700" />
          <div className="flex items-center gap-1.5 px-4 py-2 bg-indigo-500/10 text-indigo-300 text-sm font-medium rounded-3xl">
